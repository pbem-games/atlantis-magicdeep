...
class Soldier {
	public:
		...
		void RestoreItems();
      void DropItems(Battle * b);
		void Alive(int);
		void Dead(int state, ItemList *spoils, ItemList *losses);

      void CopyItems (ItemList * copyTo);
		/* Unit info */
		AString name;
		Unit * unit;
		int race;
...
class Army
{
	public:
      ...
      void Display(Battle * out, AString caption);
		void Lose(Battle *);
		void Win(Battle *);
		void Tie(Battle *);
		int CanBeHealed();
		...
		void GetMonSpoils(ItemList * spoils, int mon, int free);
		...
		int DoAnAttack( int special, int numAttacks, int attackType,
				...
				int mountBonus, int checkshield, Soldier *);
		void Kill(int);
		void Reset();
      ...
      ItemList * itemLosses;
		...
		void WriteAllLosses(Battle * b);
		void GetAmmunitionLosses();
		void GetMenLosses();      
};
