...
Soldier::Soldier(Unit * u,Object * o,int regtype,int r,int ass)
{
	...
	dskill[ATTACK_ENERGY] = 0;
	dskill[ATTACK_SPIRIT] = 0;
	dskill[ATTACK_WEATHER] = 0;
...
//		if(!attacks) attacks = 1;
	...
	if (o->capacity)
	{
		...
		{
			dskill[i] += ObjectDefs[building].defenceBonusArray[i];
		}
		if (o->runes) {
	...
	// Set up armor
	int i, item, armorType;                 // Goony & VVR
	for(i = 0; i < MAX_READY; i++)
	{
		...
		if (armor == -1  && !(ArmorDefs[armorType].flags & ArmorType::ISCLOAK)) {
                   item = unit->GetArmor(armorType, ass);
                   if (item != -1) armor = item;
                }

		if (cloakarmor == -1  && (ArmorDefs[armorType].flags & ArmorType::ISCLOAK)) {
                   item = unit->GetArmor(armorType, ass);
                   if (item != -1) cloakarmor = item;
                }

                if (armor >=0 && cloakarmor>=0) break;
	}

	if(armor == -1) {
		for(armorType = 1; armorType < NUMARMORS; armorType++) {
			if (ArmorDefs[armorType].flags & ArmorType::ISCLOAK) continue;
			item = unit->GetArmor(armorType, ass);
			if(item != -1) {
				armor = item;
				break;
	...
	if(weapon != -1)
	{
		if(WeaponDefs[weaponType].mountBonus > 0)
		{
			dskill[ATTACK_RIDING] += defenseBonus + WeaponDefs[weaponType].mountBonus;
		}
	}
	attacks = numAttacks;
}
...
int Soldier::ArmorProtect(int weaponClass)
{
	...
	int chanceArm = pArm->saves[weaponClass];
	if(chanceArm < 0) chanceArm = 0;
	if(chanceArm > getrandom(pArm->from)) return 1;

	/* 2. Check against additional armor (cloaks) */
	ArmorType *pCloak;
	int cloakType = ARMOR_NONE;
	if(cloakarmor > 0) cloakType = ItemDefs[cloakarmor].index;
	if(cloakType < ARMOR_NONE || cloakType >= NUMARMORS)
		cloakType = ARMOR_NONE;
	pCloak = &ArmorDefs[cloakType];

	int chanceCloa = pCloak->saves[weaponClass];
	if(chanceCloa < 0) chanceCloa = 0;
	if(chanceCloa > getrandom(pCloak->from)) return 1;

	/* if none saves, say "bad luck" */
	return 0;
}
...
void Soldier::RestoreItems()
{
	...
	if(cloakarmor != -1)           // Goony & VVR
		unit->items.SetNum(cloakarmor,unit->items.GetNum(cloakarmor) + 1);
	if(riding != -1)
...
void Soldier::Dead(int state, ItemList *spoils, ItemList *losses)
{
	if (state == LOSS)
   {
		// if soldier's side has lost battle, ALL items worn will go to spoils
		if (healing && healitem != -1)
      {
			if(healitem == I_HERBS)
         {
				spoils->SetNum(healitem, spoils->GetNum(healitem)+healing);
				losses->SetNum(healitem, losses->GetNum(healitem)+healing);
			}
         else if(healitem == I_HEALPOTION)
         {
				spoils->SetNum(healitem, spoils->GetNum(healitem)+healing/10);
				losses->SetNum(healitem, losses->GetNum(healitem)+healing/10);
			}
		}
		if (weapon != -1)
      {
			spoils->SetNum(weapon, spoils->GetNum(weapon) + 1);
			losses->SetNum(weapon, losses->GetNum(weapon) + 1);
      }
		if(armor != -1)
      {
			spoils->SetNum(armor, spoils->GetNum(armor) + 1);
			losses->SetNum(armor, losses->GetNum(armor) + 1);
      }
      if(cloakarmor != -1)
      {
			spoils->SetNum(cloakarmor, spoils->GetNum(cloakarmor) + 1);
			losses->SetNum(cloakarmor, losses->GetNum(cloakarmor) + 1);
      }
		if(riding != -1)
      {
			spoils->SetNum(riding, spoils->GetNum(riding) + 1);
			losses->SetNum(riding, losses->GetNum(riding) + 1);
      }
		...
		for(battleType = 1; battleType < NUMBATTLEITEMS; battleType++)
		{
         ...
         {
				int item = pBat->itemNum;
				spoils->SetNum(item, spoils->GetNum(item) + 1);
				losses->SetNum(item, losses->GetNum(item) + 1);
			}
		}
		...
		else
		{
			...
			do
			{
				if(IsSoldier(it->type))
				{
					...
					{
						if(((ItemDefs[it->type].type & IT_MAN) && u->GetFlag(FLAG_BEHIND)) ||
							(ItemDefs[it->type].type & IT_MONSTER) && MonDefs[ItemDefs[it->type].index].monbehind)
						{
							--y;
...
{
	// Returned to standart algorythm. MONSTER_NO_SPOILS set to 1000 what makes
	// released mosters virtually spoilless.
	...
	spoils->SetNum(I_SILVER,spoils->GetNum(I_SILVER) + ((silv / 2) + getrandom(silv/2)));
	...
	for (int i = 0; i < count; i++)
	{
		...
		if (i<notbehind)
		{
			...
			if (diff > 0)
			{
				if (s->damage != 0)
				{
					b->AddLine(s->name + " takes " + s->damage +
				...
				{
					b->AddLine(s->name + " takes no hits leaving it at " +
							s->hits + "/" + s->maxhits + ".");
				}
				if (s->regen)
				{
					...
					s->hits += regen;
					b->AddLine(s->name + " regenerates " + regen +
							" hits bringing it to " + s->hits + "/" +
							s->maxhits + ".");
				}
			}
	...
	for (int i=0; i<count; i++) {
		...
		else
		{
			if ((s->unit->type==U_WMON) && (ItemDefs[s->race].type&IT_MONSTER))
				GetMonSpoils(b->spoils,s->race,s->unit->free);
			s->Dead(LOSS, b->spoils, itemLosses);
		}
	}
}
...
void Army::Tie(Battle * b)
{
	...
	for(int x=0; x<count; x++) {
		...
		if (x<NumAlive()) {
			...
			s->Dead(WIN_DEAD, b->spoils, itemLosses);
		}
	}
	WriteAllLosses(b);
}
...
void Army::Win(Battle * b)
{
	...
	for(int x = 0; x < count; x++) {
		...
		else s->Dead(wintype, b->spoils, itemLosses);
	}
}
...
int Army::DoAnAttack(int special, int numAttacks, int attackType,
		...
		int mountBonus, int checkshield, Soldier *a)
{
	...
	if(canShield && checkshield) { //checkshild = 0 if attack is effect into ally soldiers
		...
		if (hi) {
			...
			if(!effect && !combat && !(SpecialDefs[special].effectflags &
			                           SpecialType::FX_NOBREAK_SHIELD))
			{
				/* We got through shield... if killing spell, destroy shield */
//				shields.Remove(hi);
//				delete hi;		// shield is not destroyed
			}
		}
	}
	...
	for(int i = 0; i < numAttacks; i++) {
		...
		if(special > 0) {
			if((SpecialDefs[special].effectflags&SpecialType::FX_NOBUILDING) &&
					tar->building) {
				tlev -= ObjectDefs[tar->building].defenceBonusArray[ attackType ];
			}
		}
		...
		/* 5. Attack soldier */
		if (checkshield && (attackType != NUM_ATTACK_TYPES)) {
			if(!((flags & WeaponType::ALWAYSREADY) || (a->unit->IsLeader() && a->unit->type != U_MAGE ))) {
				if(getrandom(2)) {      // VVR Leaders have 100% chance on attack
					continue;
				}
			}
...
void Army::WriteAllLosses(Battle * b)
{
   AString losses;
	...
	if (itemLosses->Num())
	{
		...
		losses = itemLosses->Report (1,0,1);
	}
	else losses = "nothing";

	b->AddLine(*(leader->name) + " loses " + losses + ".");

	// now list damaged units if there are any
	if (notbehind != count)
	{
		AList units;
		for (int i=notbehind; i<count; i++)
...
void Army::GetAmmunitionLosses()
{
	if(Globals->WIN_AMUNITION_LOSS && (count - NumAlive()))
	{
		for (int j = 0; j < count - NumAlive(); j++)
		{
			Soldier * temp = soldiers[j + notbehind];
			// for now the probabilities of breakdown are hardcoded.
			// 33% for weapon, 25% for mount, 50% for armor
			if (temp->weapon != -1 && !getrandom(3))
			{
				itemLosses->SetNum(temp->weapon, itemLosses->GetNum(temp->weapon) + 1);
				temp->weapon = -1;
			}
			if (temp->riding != -1 && !getrandom(4))
			{
				itemLosses->SetNum(temp->riding, itemLosses->GetNum(temp->riding) + 1);
				temp->riding = -1;
			}
			if (temp->armor != -1 && !getrandom(2))
			{
				itemLosses->SetNum(temp->armor, itemLosses->GetNum(temp->armor) + 1);
				temp->armor = -1;
			}
			if (temp->cloakarmor != -1 && !getrandom(2))
			{
				itemLosses->SetNum(temp->cloakarmor, itemLosses->GetNum(temp->cloakarmor) + 1);
				temp->cloakarmor = -1;
			}
		}
	}
...
void Army::GetMenLosses()
{
	for (int j = 0; j < count - NumAlive(); j++)
	{
		...
		if(temp->race != -1)
		{
			itemLosses->SetNum(temp->race, itemLosses->GetNum(temp->race) + 1);
		}
	}
}
