...
#define CURRENT_ATL_VER MAKE_ATL_VER( 1, 2, 0 )
...
public:
	...
	void ModifyTablesPerRuleset(void);
        ...
        // VVR
        // Current number of LesserAvatar
    int numlessav;
        // Current number of Avatar
    int numav;
	bool ModOceans();
	bool ModHills();
	bool GameFPStatistic();
...
private:
    ...
    void CreateCityNPC( ARegion *pReg, int percent, int NPCPatt );
	...
	void ModifyMonsterMaintnance(int mon, int mana, int numMon);
    ...
    void ProcessDisassembleOrder(Unit *,AString *, OrdersCheck *pCheck);        // VVR
    void ProcessTownOrder(Unit *, AString *, OrdersCheck *pCheck);              // VVR
    void ProcessChangeOrder(Unit *, AString *, OrdersCheck *pCheck);            // VVR
    ...
    void ProcessLast_Order(Unit *,AString * , OrdersCheck *pCheck);             //DSN & VVR
    ...
    void ProcessManauseOrder(Unit *, AString *, OrdersCheck *pCheck);  // VVR
    ...
    int CountUnusedPowerPoints(Faction *); // [ufas]
    int CountLeaders (Faction *f); // [ufas]

    void FindDeadFactions();
    void DeleteEmptyUnits();
    void CalcSupportMonsters();         // VVR
    void ClearFacSupmon();              // VVR
    void SetRebelions(ARegion *, int);  // VVR
    ARegion * GetRegionForNexusMove(int);     // VVR
    void PersonalPoolPostTurn();
    ...
    // [ufas]
    int CountTaxActivity(Faction *f);
    int CountTradeActivity(Faction *f);
    int CalcMagesPPCost(Faction *f);
    int CalcLeadersPPCost(Faction *f, int add);
    int CountPossibleFreeLeaders(Faction *f);
    int CalcApprenticesPPCost(Faction *f);
    int MagePPCost(int);
    int ApprenticePPCost(int);
    ...
    //  VVR
    bool CalcCastMana(Unit *u, int skill, int level);
    bool CheckCostCast(Unit *u, int skill, int level);
    void ARMAddAncientBuildings(int maplevel); // Klaus
    ...
    void RunDisassembleOrders(ARegion *);       // VVR
    void RunTownOrders(ARegion *);              // VVR
    void RunUnitTown(ARegion *, Unit *);        // VVR
    void ManaGeneration(Unit *u, int valminus); // VVR
