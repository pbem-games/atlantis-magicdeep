...
void Game::CreateWorld()
{
	...
	Awrite("");
	Awrite("Creating Nexus level...");
	regions.CreateNexusLevel( 0, nx, ny, "nexus" );
	Awrite("Nexus level created.");

	Awrite("--------");
	Awrite("");
	Awrite("Creating surface level...");
  ...
  do
  {
	...
	while(alg <= 0)
	{
		...
		Awrite("3 - Import from map.csv");
		alg = Agetint();
		if(alg < 1 || alg > 3)
		{
			alg = 0;
			Awrite( "Wrong value. Please enter `1`, `2` or `3`:" );
		}
	}
	if(alg == 1)
	{
		...
		while (contiSize <= 0)
		{
			Awrite("How big would be continents? ");
			contiSize = Agetint();
		}
		regions.CreateSurfaceLevel( 1, xx, yy, ocean, contiSize, 0 );
	}
	else
	{
		if(alg == 2)
		{
			regions.CreateElementalWorld(1, xx, yy, 0);
		}
		else
		{
			success = regions.ImportWorld(1, xx, yy, 0);
		}
	}
  }
  while(!success);

	Awrite("Surface level created.");
	...
	for(i = 2; i < Globals->UNDERWORLD_LEVELS+2; i++) {
		...
		Awrite("--------");
		Awrite("");
		Awrite(AString("Creating underworld level ") + AString(i) + AString("..."));
		Awrite("Which algorithm should be used?");
		alg = 0;
		while(alg <= 0)
		{
		...
		if(alg == 1)
		{
			pStr = new AString("underworld");
			regions.CreateUnderworldLevel(i, xx/xs, yy/ys, pStr->Str());
		}
		else
		{
			Awrite("Please, enter the name for that level:");
			pStr = AGetString();
			regions.CreateElementalWorld(i, xx, yy, pStr->Str());
		}
		Awrite(AString("Level of ") + pStr->Str() +  AString(" created"));
	}
	// Underdeep levels
	for(i=Globals->UNDERWORLD_LEVELS+2;
		...
		Awrite("--------");
		Awrite("");
		Awrite(AString("Creating underdeep level ") + AString(i) + AString("..."));
		Awrite("Which algorithm should be used?");
		alg = 0;
		while(alg <= 0)
		{
		...
		if(alg == 1)
		{
			pStr = new AString("underdeep");
			regions.CreateUnderdeepLevel(i, xx/xs, yy/ys, pStr->Str());
		}
		else
		{
			Awrite("Please, enter the name for that level:");
	...
	if(Globals->ABYSS_LEVEL) {
		Awrite("--------");
		Awrite("");
		Awrite(AString("Creating abyss level ") + AString(Globals->UNDERWORLD_LEVELS +
				Globals->UNDERDEEP_LEVELS + 2) + AString("..."));
		regions.CreateAbyssLevel(Globals->UNDERWORLD_LEVELS +
				Globals->UNDERDEEP_LEVELS + 2, "abyss");
		Awrite("Abyss level created");
	}
