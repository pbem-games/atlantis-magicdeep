...
{
    pFac->unclaimed = Globals->START_MONEY + (TurnNumber() -1) * 50;
    pFac->mana = 10;
    ...
    temp2->Study(S_NATURE_LORE, 30);
    temp2->Study(S_MANA_GENERATION, 30);
    temp2->reveal = REVEAL_FACTION;

    if ( Globals->PERSONAL_MANA )
      { temp2->manause = 2; pFac->mana = 0; }
    else { temp2->manause = 0; }

    if (TurnNumber() >= 12) {
        temp2->Study(S_FORCE, 30);
        temp2->Study(S_MANA_GENERATION, 90);
        pFac->mana = 20;
    }
