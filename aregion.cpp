...
// если включены многонациональные города, но в городе нет
// наёмников других рас, то добавляем их.
...
//  сохраняем старую расу, если она может жить в новом типе местности
	int oldRace, newRace;
	oldRace = race;
	race = -1;
	int i;
...
//  если старая раса не может жить в новом типе местности, то назначаем новую расу.
	while (race == -1 || (ItemDefs[race].flags & ItemType::DISABLED)) {
		int n = getrandom(IsCoastal() ? allraces : noncoastalraces);
		if(n > noncoastalraces-1) {
			race = typer->coastal_races[n-noncoastalraces-1];
