		...
		OrdersCheck *pCheck)
{
	switch(orderNum) {
		...
		case O_CAST:
			...
			break;
		case O_CHANGE:
			ProcessChangeOrder(unit, o, pCheck);    // VVR
			break;
		case O_CLAIM:
		...
		case O_FORGET:
			...
			break;
		case O_MANAUSE:                                 // VVR
			ProcessManauseOrder(unit, o, pCheck);
			break;
		case O_WITHDRAW:
		...
		case O_PRODUCE:
			...
			break;
		case O_DISASSEMBLE:             // VVR
			ProcessDisassembleOrder(unit, o, pCheck);
			break;
		case O_TOWN:                    // VVR
			ProcessTownOrder(unit, o, pCheck);
			break;
		case O_PROMOTE:
...
void Game::ProcessChangeOrder(Unit * u, AString * o, OrdersCheck *pCheck)
{
	if(pCheck) return;
...
void Game::ProcessStealOrder(Unit * u,AString * o, OrdersCheck *pCheck)
{
	...
	if (IsSoldier(i) || ( ItemDefs[i].type & IT_ETHEREAL) ) {
		...
		if (!pCheck) {
			...
			if (TerrainDefs[reg->type].similar_type == R_OCEAN){
				...
				return;
      	                ...
      	                if (ObjectIsShip(ot)) {
				if (!reg->IsCoastalOrLakeside() && !(ObjectDefs[ot].flags &
						ObjectType::SHIP_AIRBORN)) {
...
void Game::ProcessProduceOrder(Unit * u,AString * o, OrdersCheck *pCheck)
{
	...
	u->monthorders = p;
                ...
                // VVR
void Game::ProcessDisassembleOrder(Unit *u, AString *o, OrdersCheck *pCheck)
{
	AString * token = o->gettoken();
	if (!token) {
		ParseError(pCheck, u, 0, "DISASSEMBLE: Number to disassemble not given.");
		return;
	}
        int num = token->value();
	delete token;
	token = o->gettoken();
	if (!token) {
		ParseError(pCheck, u, 0, "DISASSEMBLE: Item not given.");
		return;
	}
        int it = ParseEnabledItem(token);
	delete token;

	DisassembleOrder * d = new DisassembleOrder;
        d->num = num;
	d->item = it;
        d->skill = ItemDefs[it].pSkill;
       	d->level = ItemDefs[it].pLevel;
        for(int i=0; i < 4; i++) {
                d->mat[i][0] = ItemDefs[it].pInput[i].item;
                d->mat[i][1] = ItemDefs[it].pInput[i].amt;
        }
	...
	if (u->monthorders ||
		(Globals->TAX_PILLAGE_MONTH_LONG &&
		 ((u->taxing == TAX_TAX) || (u->taxing == TAX_PILLAGE)))) {
		if (u->monthorders) delete u->monthorders;
		AString err = "DISASSEMBLE: Overwriting previous ";
		if (u->inTurnBlock) err += "DELAYED ";
		err += "month-long order.";
		ParseError(pCheck, u, 0, err);
	}
	...
	u->monthorders = d;
                ...
                // VVR
void Game::ProcessTownOrder(Unit *u, AString *o, OrdersCheck *pCheck)
{
	AString * token = o->gettoken();
        int flag = 0;
	if (token) {
                ...
                else flag = 1;          // buld city = 1
        }
        else flag = 2;                  // build city without name = 2

	TownOrder * order = new TownOrder;
        order->flag = flag;
        if(flag == 1) order->name = token;
	if (u->monthorders ||
		(Globals->TAX_PILLAGE_MONTH_LONG &&
		 ((u->taxing == TAX_TAX) || (u->taxing == TAX_PILLAGE)))) {
		if (u->monthorders) delete u->monthorders;
		AString err = "TOWN: Overwriting previous ";
		if (u->inTurnBlock) err += "DELAYED ";
		err += "month-long order.";
		ParseError(pCheck, u, 0, err);
	}
...
{       // declare <attitude> <faction1> [faction2] [faction3] ...
        int er = 0;

	AString * token = o->gettoken();
	if (!token) {
		ParseError(pCheck, 0, f, "DECLARE: No attitude given.");
		return;
	}
        ...
        do {
        	...
        	if((!pCheck) && (!er)) {
	        	if (fac == -1) {
               	...
               	token = o->gettoken();
                if(token) fac = token->value();
                er = 0;
        } while(token);
}
//  +++ Origin Declare command +++ //
/*
	AString * token = o->gettoken();
	if (!token) {
		ParseError(pCheck, 0, f, "DECLARE: No faction given.");
...
void Game::ProcessWithdrawOrder(Unit *unit, AString *o, OrdersCheck *pCheck)
{
    ...
    if ( !( (ItemDefs[item].type & IT_NORMAL) || ((ItemDefs[item].type &
                        IT_MAN) && Globals->ALLOW_WITHDRAW_MEN) ) ) {
        ParseError(pCheck, unit, 0, "WITHDRAW: Invalid item.");
        return;
    }
    if(item == I_SILVER) {
	...
	if (*token == "village" || *token == "town" || *token == "city") {
		...
		if (!pCheck) {
			...
			switch(towntype) {
				...
				case TOWN_TOWN:
					if(unit->object->type == O_KEEP) ok = 1;
					if(unit->object->type == O_MKEEP) ok = 1;
				case TOWN_CITY:
...
void Game::ProcessManauseOrder(Unit *u, AString *o, OrdersCheck *pCheck)
{
	/* This is an instant order */
	AString * token = o->gettoken();
	if (!token) {
		ParseError(pCheck, u, 0, "MANAUSE: Invalid argument.");
		return;
	}
	int val = -1;

        if(Globals->PERSONAL_MANA && ((*token == AString("personal")) ||
		(*token == AString("PERSONAL")))) val = 2;
        if((*token == AString("mpot")) ||
		(*token == AString("MPOT"))) val = 1;
        if(!Globals->PERSONAL_MANA && ((*token == AString("mana")) ||
		(*token == AString("MANA")))) val = 0;
	if (val == -1) {
		ParseError(pCheck, u, 0, "MANAUSE: Invalid argument.");
		return;
	}
...
void Game::ProcessAdvanceOrder(Unit * u,AString * o, OrdersCheck *pCheck)
{
	MoveOrder *m = 0;
        ...
        if(u->monthorders && u->monthorders->type == O_STUDY) {
                order = (StudyOrder *) u->monthorders;
        	sk = order->skill;
        }

        if (u->monthorders && (u->monthorders->type == O_STUDY) && (SkillDefs[sk].flags & SkillType::MAGIC)) {
                u->monordmagestudy = order;
                u->monthorders = 0;
        }
