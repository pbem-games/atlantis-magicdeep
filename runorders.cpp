...
// 2003/Aug/21 Vitaliy Roskiy (VVR)     Added ChangeOrder - change num for a ship
//
#include "game.h"
#include "gamedata.h"
...
void Game::RunOrders()
{
	...
	RunFindOrders(); // disabled I_SHORTBOW
	...
	forlist(&regions)
	{
		...
		forlist(&r->objects)
		{
			...
			forlist(&o->units)
			{
				...
				{
					if(((u->type == U_MAGE) || (u->type == U_GUARDMAGE)) && !u->teleportorders)
						ManaGeneration(u, 0);
				}
			}
		}
...
int Game::CountLeaders( Faction *pFac )
{
	int i = 0;
	forlist( &regions ) {
		ARegion * r = (ARegion *) elem;
		forlist( &r->objects ) {
			...
			forlist(&o->units) {
				Unit * u = (Unit *) elem;
				if (u->faction == pFac && u->IsLeader() && u->type != U_MAGE && u->type != U_APPRENTICE) i+= u->GetMen();
			}
		}
	}
	return( i );
...
int Game::TaxCheck( ARegion *pReg, Faction *pFac )
{
        if( AllowedTaxes( pFac ) == -1 ||
                ...
                Globals->FACTION_LIMIT_TYPE != GameDefs::FACLIM_POWER_POINTS) ) {
                // No limit either for this faction type or in whole game.
                return 1;
        }
		...
		if((Globals->FACTION_LIMIT_TYPE == GameDefs::FACLIM_FACTION_TYPES &&
			CountTaxActivity(pFac) + pReg->RegionTaxSize() > AllowedTaxes(pFac)) ||
		   (Globals->FACTION_LIMIT_TYPE == GameDefs::FACLIM_POWER_POINTS &&
			CountUnusedPowerPoints(pFac) < pReg->RegionTaxSize()))
		{
				// No availible tax regions or power points.
			return 0;
		}
		else
		{
...
int Game::TradeCheck( ARegion *pReg, Faction *pFac )
{
        ...
        forlist( &( pFac->trade_regions )) {
                ARegion *x = ((ARegionPtr *) elem)->ptr;
                if( x == pReg )
                {       // This faction already performed a trade action in this
                        // region.
                        return 1;
                }
        }
        if((Globals->FACTION_LIMIT_TYPE == GameDefs::FACLIM_FACTION_TYPES &&
			CountTradeActivity(pFac) + pReg->RegionTradeSize() > AllowedTrades(pFac)) ||
            (Globals->FACTION_LIMIT_TYPE == GameDefs::FACLIM_POWER_POINTS &&
            CountUnusedPowerPoints(pFac) < pReg->RegionTradeSize()))
        {
                // No availible trade regions or power points.
			return 0;
        }
        else
        {
                // Add this region to the faction's trade list.
                ARegionPtr *y = new ARegionPtr;
                y->ptr = pReg;
...
// dsnlab & VVR
  ...
  {
      return TradeCheck(pReg, pFac); // [ufas] Variable cost maybe should do.
  }
...
int Game::TradeCheckRoadbuild (ARegion *pReg, Faction *pFac)
{
  if( Globals->FACTION_LIMIT_TYPE == GameDefs::FACLIM_POWER_POINTS)
  {
      return (TradeCheck(pReg, pFac) || TaxCheck(pReg, pFac) );
      // [ufas] Road building is a general activity, trade or tax.
      // [ufas] BUG - counts as trade regions even if its trade.
  }
...
{
        int need = ObjectDefs[o->type].cost;
	...
	if (!u->GetMen()) {
		...
		return;
       	...
       	if ((2*u->GetMen()) < need) {
		u->Error("DESTROY: More units need to destroy this structure.");
		forlist(&o->units) {
			((Unit *) elem)->destroy = 0;
		}
		return;
	...
	if (o->CanModify() && !o->CantDestroy() && (need > 0)) {
...
void Game::RunFindUnit(Unit * u)
{
	...
	forlist(&u->findorders) {
		...
		if(!all) {
			...
			if (fac) {
				u->faction->Event(AString("Faction ") + *(fac->name));
	        			...
	        			Unit * u = (Unit *) elem;
		                		...
		                		if(u && u->change) {
	        		                ...
	        		                o->SetName(new AString("Ship"));
                		                u->change = 0;
                                                u->Event("Number of the ship is changed.");
				                }
                                }
			}
		}
			...
			forlist ((&u->buyorders)) {
				...
				if (o->item == m->item) {
					if (ItemDefs[o->item].type & IT_MAN) {
						...
						if (o->item==I_LEADERS && Globals->FACTION_LIMIT_TYPE == GameDefs::FACLIM_POWER_POINTS &&
                  	(Globals->UFAS_OPTIONS & GameDefs::LEADERS_COST_PP))
						{
							int num = o->num;
							int addlead = CalcLeadersPPCost(u->faction, num);
							int lead = CalcLeadersPPCost(u->faction, 0);
							if ((addlead - lead) > CountUnusedPowerPoints(u->faction))
                     {
								u->Error("BUY: Faction has too many leaders.");
								o->num = 0;
							}
						}
...
int Game::DoWithdrawOrder(ARegion * r,Unit * u, WithdrawOrder * o)
{
	int itm = o->item;
	int amt = o->amount;
	int cost = (ItemDefs[itm].baseprice * 3 / 2) * amt;
	...
	if (u->type == U_MAGE && (ItemDefs[itm].type & IT_MAN)) {
		u->Error("WITHDRAW: Mages cannot withdraw men.");
		return 0;
	}

	if (u->type == U_APPRENTICE && (ItemDefs[itm].type & IT_MAN)) {
		u->Error("WITHDRAW: Apperntices cannot withdraw men.");
		return 0;
	}

	if ((itm == I_LEADERS && u->IsNormal()) || (itm != I_LEADERS &&
			(ItemDefs[itm].type & IT_MAN) && u->IsLeader())) {
		u->Error("WITHDRAW: Cannot mix leaders and normal men.");
		return 0;
	}

   if (itm == I_LEADERS && Globals->FACTION_LIMIT_TYPE == GameDefs::FACLIM_POWER_POINTS &&
   	(Globals->UFAS_OPTIONS & GameDefs::LEADERS_COST_PP))
   {
   	int addlead = CalcLeadersPPCost(u->faction, amt);
   	int lead = CalcLeadersPPCost(u->faction, 0);
   	if ((addlead - lead) > CountUnusedPowerPoints(u->faction))
   	{
   		u->Error("WITHDRAW: Faction has too many leaders.");
   		return 0;
   	}
   }
    ...
    if (cost < 0) {
        u->Error(AString("WITHDRAW: Can't withdraw ")+
                ItemString(itm,amt)+".");
        return 0;
    }

    u->faction->unclaimed -= cost;
        if (ItemDefs[itm].type & IT_MAN)
                u->Event(AString("Hires ") + ItemString(o->item,amt) + " via WITHDRAW order.");
        else
...
int Game::DoGiveOrder(ARegion * r,Unit * u,GiveOrder * o)
{
	...
	if (amt == -1) {
		/* Give unit */
   	// [ufas] TODO: add leaders' PP cost support\restrictions
		if (u->type == U_MAGE)
      {
      	if(Globals->FACTION_LIMIT_TYPE != GameDefs::FACLIM_UNLIMITED)
         {
      		...
      		if ( Globals->FACTION_LIMIT_TYPE == GameDefs::FACLIM_POWER_POINTS)
      		{
					t->faction->nummages = CountMages(t->faction);
         		if (CountUnusedPowerPoints(t->faction) < MagePPCost(t->faction->nummages + 1))
      			{
      				u->Error("GIVE: faction has not enough unused power points to have another magician.");
         			return 0;
      			}
      		}
      		else
		...
		if(u->type == U_APPRENTICE)
      {
			if(Globals->FACTION_LIMIT_TYPE != GameDefs::FACLIM_UNLIMITED)
         {
         	...
         	if ( Globals->FACTION_LIMIT_TYPE == GameDefs::FACLIM_POWER_POINTS)
         	{
					t->faction->numapprentices = CountApprentices(t->faction);
					if (CountUnusedPowerPoints(t->faction) < ApprenticePPCost(t->faction->numapprentices + 1))
         		{
         			u->Error("GIVE: faction has not enough unused power points to have another apprentice.");
         			return 0;
         		}
         	}
         	else
      ...
      if (u->IsLeader() && Globals->FACTION_LIMIT_TYPE == GameDefs::FACLIM_POWER_POINTS &&
      		(Globals->UFAS_OPTIONS & GameDefs::LEADERS_COST_PP))
      {
			int num = u->GetMen();
         int addlead = CalcLeadersPPCost(t->faction, num);
         int lead = CalcLeadersPPCost(t->faction, 0);
         if ((addlead - lead) > CountUnusedPowerPoints(t->faction)) {
				u->Error("GIVE: Faction has too many leaders.");
            return 0;
         }
      }
	...
	if((o->item == I_BALROG) && (t->items.GetNum(I_BALROG) > 0)) {
		u->Error(AString("GIVE: Can't give. Target unit have balrog."));
		return 0;
	}
				...
				if (u->guard == GUARD_SET || u->guard == GUARD_GUARD) {
					...
					if (u->type != U_GUARD && r->HasCityGuard()) {
						...
						continue;
                                        ...
                                        Unit *u2 = r->IsGuarded1();
                                        bool uatt = 1;
                                        if(u2) uatt = 0;
                                        if(u2 && u2->faction->GetAttitude(u->faction->num) == A_ALLY) uatt = 1;
	                                if(!uatt && u->guard == GUARD_SET) {
                                                u->guard = GUARD_NONE;
...
void Game::DeleteEmptyInRegion(ARegion * region)
{
	forlist(&region->objects) {
        ...
        // VVR
void Game::CalcSupportMonsters()
{
        int facmana = 0, supcost = 0 , numsup = 0, numumon = 0;
        int musesup = 0;
        int umana = 0;
        double calcm;
			...
			forlist((&obj->units)) {
				Unit * u = (Unit *) elem;
                                if(u->type != U_WMON && u->type != U_GUARD && u->type != U_GUARDMAGE) {
                                        forlist((&u->items)) {
                                                Item * it = (Item *) elem;
                                                if(!(ItemDefs[it->type].type & IT_MONSTER)) continue;

                                                supcost = MonDefs[ItemDefs[it->type].index].msup;
                                                if(supcost < 1) continue; // why ??

                                                numsup = MonDefs[ItemDefs[it->type].index].numsup;
                                                numumon = u->items.GetNum(it->type);
                                                calcm = (double)numumon * supcost / numsup;
                                                if ( Globals->PERSONAL_MANA )
                                                {
                                                        calcm *= 10; // 1 "faction" mana = 10 "personal" mana
                                                }
                                                int ct = calcm;

                                                u->faction->supmtemp += calcm - ct;
                                                if(u->faction->supmtemp >= 0.5) {
                                                        calcm++;
                                                        u->faction->supmtemp--;
                                                }
                                                calcm = (int)(calcm);
                                                ...
                                                if ( Globals->PERSONAL_MANA )
                                                {
                                                        umana = u->items.GetNum(I_MANA);
                                                }
                                                else
                                                        facmana = u->faction->mana;

                                                //umana = u->items.GetNum(I_MANAPOTION); // trash ??
                                                ...
                                                if ( Globals->PERSONAL_MANA )
                                                {
                                                  if(umana >= calcm) {
                                                    umana -= calcm;
                                                    u->items.SetNum(I_MANA, umana);
                                                  } else vanish = 1;

                                                } else
                                                if(facmana >= calcm) {
                                                        u->faction->mana -= calcm;
                                                }
                                                else vanish = 1;
                                                ...
                                                if (vanish) {
                                                        ...
                                                        int bmons = 0;
                                                        if  (Globals->PERSONAL_MANA) {
                                                          bmons = int ( (double)(getrandom(100))/100 + (double)numumon * (1 - (double)umana / calcm));
                                                          calcm = umana;
                                                          u->items.SetNum(I_MANA, 0);
                                                        }
                                                        else
                                                        {
                                                          bmons = int ( (double)(getrandom(100))/100 + (double)numumon * (1 - (double)facmana / calcm));
                                                          calcm = facmana; // foooo??
                                                          u->faction->mana = 0;
                                                        }
                        				...
                        				Faction *mfac = GetFaction(&factions,monfaction);
                                       			if(!Globals->WANDERING_MONSTERS_EXIST) {
                                                                u->items.SetNum(it->type,numumon-bmons);
                                                        }
                                                        else {
      		        		                	Unit *mon = GetNewUnit( mfac, 0 );
                       	        				mon->MakeWMon(MonDefs[ItemDefs[it->type].index].name,it->type,bmons);
                       			        		mon->MoveUnit( r->GetDummy() );
	                        		        	u->items.SetNum(it->type,numumon-bmons);
	        	                		...
	        	                		monname = (bmons==1)?ItemDefs[it->type].name:ItemDefs[it->type].names;

                                       			if(!Globals->WANDERING_MONSTERS_EXIST) {
                                				u->Event(AString(bmons) + " summoned " + monname + " vanish.");
                                                        }
                                                        else {
                                				u->Event(AString(bmons) + " controlled " + monname + " break free!");
                                                        }
                                                }
                                                musesup += calcm;
                                        }
                                        if(musesup)
                                                u->Event(AString( (Globals->WANDERING_MONSTERS_EXIST)?"Controlled":"Summoned") +
                                                " monsters consumes " + AString(musesup) + " mana for maintance.");
                                        musesup = 0;
                                }
			}
		}
...
void Game::ClearFacSupmon()
{
        forlist(&factions) {
                Faction * fac = (Faction *) elem;
                if (!fac->IsNPC() && !fac->exists) fac->supmtemp = 0;
        }
}

void Game::PersonalPoolPostTurn ()
{
    forlist((&regions)) {
        ARegion * r = (ARegion *) elem;
        forlist((&r->objects)) {
            ...
            forlist((&obj->units)) {
                Unit * u = (Unit *) elem;
                int num = u->items.GetNum(I_MANA);
                if (num) {

                    // if have mana...

                    // Check skills
                    int canhold = u->GetSkill(S_MANA_GENERATION) * 5 +
                                    u->GetSkill(S_CHANNEL) * 10 +
                                    u->GetSkill(S_PATTERN) * 3 +
                                    u->GetSkill(S_FORCE)   * 3 +
                                    u->GetSkill(S_SPIRIT)  * 3;

		    // canhold *= canhold; // squared by demand (and canceled) (;

		    int losses = 0;
                    
                    if ( canhold < num )
                    {
                        losses = (num - canhold + getrandom(2)) / 2;
                    }// else losses = (num + getrandom(10)) / 10;

                    if ( losses > 0 )
                    {
                      u->items.SetNum(I_MANA, num - losses);
                      u->Event(AString(losses) + " personal mana dissipates.");
                    }
                }
            }
        }
