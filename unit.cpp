...
//        water.
// 2003/Oct/26  A. Gazhariga    Fixed some bugs with monsters advance command:
//                              1. peaceful monsters are now never advances
//                              2. avoid flag resets to default each turn
...
Unit::Unit()
{
  ...
  manause = Globals->PERSONAL_MANA * 2; // (; wiz
...
{
        if(ItemDefs[mon].flags & ItemType::DISABLED) return;
        // Never create monsters that are disabled
...
void Unit::Readin(Ainfile *s, AList *facs, ATL_VER v)
{
  ...
  // return monster spoils to standart
  if(free < 0) free = Globals->MONSTER_NO_SPOILS + Globals->MONSTER_SPOILS_RECOVERY;
}
...
AString Unit::MageReport()
{
  ...
  if (combat != -1) {
    temp = AString(". Combat spell: ") + SkillStrs(combat);
        ...
        temp += AString(". Mana source: ");
        //temp += (manause)?"mana potions":"faction pool";
        switch ( manause ){
                case 0: temp += "faction pool"; break;
                case 1: temp += "mana potions"; break;
                case 2: temp += "personal pool"; break;
        }
        forlist(&skills) {
                Skill * s = (Skill *) elem;
                if(SkillDefs[s->type].costsupply) pp += SkillDefs[s->type].costsupply;
        }
        temp += AString(". Support: ");
        temp += AString(pp) + "PP";
        return temp;
}
         ...
         int autosee)
{
  ...
  if (obs == 2) {
    ...
    temp += AString(SwimmingCapacity());
                temp += ". Real stealth: ";
                temp += AString(GetSkill(S_STEALTH));
    temp += ". Skills: ";
...
AString * Unit::BattleReport(int obs)
{
  ...
  *temp += lvl;
  ...
  {
    *temp += ", Spell: ";
    *temp += SkillDefs[combat].name;
    *temp += " ";
    *temp += GetRealSkill(combat);
  }
...
void Unit::ClearOrders()
{
  ...
  if (monthorders) delete monthorders;
  monthorders = 0;
  if (monordmagestudy) delete monordmagestudy;
  monordmagestudy = 0;
  inTurnBlock = 0;
  presentTaxing = 0;
  if (presentMonthOrders) delete presentMonthOrders;
...
void Unit::DefaultOrders(Object * obj)
{
  ...
  if (type == U_WMON) {
    ...
    combat = NPCDefs[npctype].combatSpell[object->region->GetGuardLevel()];
  } else{
...
{
  if (type == U_WMON) {
                SetMonFlags();// returning 'avoid' flag to it's normal state,
                              // probably corrupted by previous 'advance' order
    forlist(&items) {
      Item *i = (Item *) elem;
...
// VVR
int Unit::GetMinusStealth()
{
        int endw = Globals->SPY_WEIGHT;
        int beginw = 0;
        int uweight = Weight() / GetMen();
        bool fl = true;
        int retsteath = 0;
        int raz;

		if(Globals->SPY_WEIGHT > 0)
		{
			while(fl)
			{
				if((uweight > beginw) && (uweight <= endw))
					fl = false;
				else
				{
					retsteath++;
...
void Unit::ForgetSkill(int sk)
{
  ...
  forlist(&skills) {            // VVR forget all skill by family skill
      Skill * s = (Skill *) elem;
      if(SkillDefs[s->type].depends[0].skill == sk) ForgetSkill(s->type);
      if(SkillDefs[s->type].depends[1].skill == sk) ForgetSkill(s->type);
      if(SkillDefs[s->type].depends[2].skill == sk) ForgetSkill(s->type);
  }
...
int Unit::Study(int sk,int days,bool pract)
{
	Skill *s, *cur;
	int nums = Globals->SKILL_LIMIT_NONLEADERS;
	bool resk = 0;
	int mantype = -5;
	...
	if(!IsLeader() && nums > 0)
	{
		if (skills.Num() == nums)
		{
			...
			for(int i=1; i <= nums; i++)
			{
				...
				s = (Skill *) skills.Next(s);
			}
			if(!resk)
			{
				AString str;
				str = "STUDY: Can know only ";
				str += (AString) nums;
				str += " skills.";
				Error(str);
				return 0;
	...
	forlist(&skills)
	{
		...
		forlist(&items)
		{
			...
			if (ItemDefs[i->type].type & IT_MAN)
			{
				int m = SkillMax(s->type, i->type, vsmax);
				if (m < max)
				{
					max = m;
					mantype = i->type;
	...
	if(mantype != -5 && vsmax == -1)
	{
	// Check on special skill for unique max (5 or 6)
		int mant = ItemDefs[mantype].index;
		for(unsigned int c=0; c < sizeof(ManDefs[mant].skills) / sizeof(ManDefs[mant].skills[0]); c++)
		{
			if(ManDefs[mant].skills[c] == sk)
			{
				if(skills.GetDays(sk) > GetMen() *
				   GetDaysByLevel(ManDefs[mant].speciallevel))
				{
					vsmax = sk;
					vsdays = skills.GetDays(sk);
				}
			}
		}
	}
...
{
  // dirty hack to prevent practise of combat skills.
  if ((SkillDefs[sk].flags & SkillType::COMBAT) || sk == S_COMBAT ||
       sk == S_TACTICS || sk == S_OBSERVATION || sk == S_RIDING ||
       sk == S_STEALTH || sk == S_HEALING || sk == S_MAGICAL_HEALING)
    return 0;
  ...
  men = GetMen();
  // possible bug if raising skill limit over level 5:
...
void Unit::AdjustSkills()
{
	...
	if(IsLeader())
	{
		...
		{
			Skill * s = (Skill *) elem;
			if (GetRealSkill(s->type) >= SkillMax(s->type,I_LEADERS,vsmax))
			{
				s->days = GetDaysByLevel(SkillMax(s->type,I_LEADERS,vsmax)) * GetMen();
			}
		}
	}
	else
	{
		...
		{
			//
			// Not a leader, can only know SKILL_LIMIT_NONLEADERS skill
			//
			if (skills.Num() > Globals->SKILL_LIMIT_NONLEADERS)
			{
				//
				// Find slowest skills and eliminate
				//
				int raz = skills.Num() - Globals->SKILL_LIMIT_NONLEADERS;
		...
		forlist(&skills)
		{
			...
			forlist(&items)
			{
				...
				if (ItemDefs[i->type].type & IT_MAN)
				{
					if (SkillMax(theskill->type,i->type,vsmax) < max)
					{
						max = SkillMax(theskill->type,i->type,vsmax);
					}
				}
			}
...
void Unit::CheckMergeMax()
{
        forlist(&skills) {
           Skill *s = (Skill *)elem;
          if ((s->type != vsmax) || (s->type == -1)) continue;
                if(s->days < vsdays) {
                        vsmax = -1;
                        vsdays = 0;
                        break;
...
int Unit::MaintCost()
{
  ...
  switch(Globals->MULTIPLIER_USE) {
    case GameDefs::MULT_NONE:
      ...
      if(type == U_MAGE) retval = Globals->MAGE_COST;
      break;
...
int Unit::FlyingCapacity()
{
  ...
  forlist(&items) {
    Item * i = (Item *) elem;
//                if((i->type == I_WHORSE) &&     // VVR, No fly on WING without RIDI 3+
//                        (GetSkill(S_RIDING) < MountDefs[ItemDefs[I_WHORSE].index].minBonus)) continue;
...
int Unit::MoveType(ARegion * r1,ARegion * r2, int dir)
{
  int weight = items.Weight();

  bool possibleRiding;
  possibleRiding = false;
  if(r1->HasExitRoad(dir) && r2->HasConnectingRoad(dir))
    possibleRiding = true;
  if(r1->HasExitMRoad(dir) && r2->HasConnectingMRoad(dir))
    possibleRiding = true;
  if(TerrainDefs[r2->type].flags & TerrainType::RIDINGMOUNTS)
    possibleRiding = true;
  ...
  if(TerrainDefs[r2->type].flags & TerrainType::FLYINGMOUNTS)
    possibleFlying = true;
  if(!Globals->ADVANCED_MOVEMENT)
    possibleFlying = true;


  if (CanFly(weight) && possibleFlying) return M_FLY;
  if (CanRide(weight) && possibleRiding) return M_RIDE;
...
int Unit::CalcMovePoints(ARegion * r1,ARegion * r2, int dir)
{
  switch (MoveType(r1, r2, dir)) {
    case M_NONE:
      return 0;
    case M_WALK:
      if(Globals->ADVANCED_MOVEMENT)
        return Globals->FLY_SPEED/2;
      else
...
int Unit::GetAttitude(ARegion * r,Unit * u)
{
  ...
  /* units of friendly or ally faction are treated properly even if they are
     unseen or their faction is undetected */
  ...
  switch(sk) {
    case S_OBSERVATION:
      ...
      if ((bonus <= (2 + Globals->IMPROVED_AMTS)) &&
        ...
        if(GetFlag(FLAG_TRUES)) bonus ++;
      } else {
                if(!GetSkill(S_TRUE_SEEING) && GetFlag(FLAG_TRUES)) bonus ++;
      }
      break;
    case S_STEALTH:
...
int Unit::CanUseWeapon(WeaponType *pWep)
{
  ...
  // VVR
int Unit::AddCostForNumSkill(int sk)
{
        if(IsLeader()) return 0;

        if(!skills.GetDays(sk)) return skills.Num() * 10;

        return (skills.Num() - 1) * 10;
}

void Unit::AdjustSpecialSkill()
{
	Skill *s;
	int race = -1;
	int minlev = 1000;
	forlist(&skills)
	{
		...
		forlist(&items)
		{
			...
			if (ItemDefs[i->type].type & IT_MAN)
			{
				int tmp = SkillMax(s->type, i->type, vsmax);
				if (tmp < minlev)
				{
					// set min to the minimum level allowed for any skill unit has
					minlev = tmp;
					// race is race that caused the strongest limitation
					race = i->type;
				}
			}
		}

		// if any limits present and special skill not set
		if(race >= 0 && vsmax == -1)
		{
			int mant = ItemDefs[race].index;
			for(unsigned int c=0; c < sizeof(ManDefs[mant].skills) /
			                     sizeof(ManDefs[mant].skills[0]); c++)
			{
				if(ManDefs[mant].skills[c] == s->type)
				{
					if(skills.GetDays(s->type) > GetMen() *
					   GetDaysByLevel(ManDefs[mant].speciallevel))
					{
						vsmax = s->type;
						vsdays = skills.GetDays(s->type);
						return;
					}
				}
			}
