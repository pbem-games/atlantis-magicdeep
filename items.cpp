...
AString ShowSpecial(int special, int level, int expandLevel, int fromItem)
{
	...
	if(spd->effectflags & SpecialType::FX_DEFBONUS) {
        ...
        if(spd->effectflags & SpecialType::FX_NOBREAK_SHIELD) {
                temp += " This ability cannot destroy appropriate shield.";
        }
	/* Now the damages */
	for(i = 0; i < 4; i++) {
		...
		if(!expandLevel) {
			temp += " times ";
                        if(spd->targproc != -1) {
                                temp += AString("plus ") + spd->targproc + " % of first line target army ";
                        }
                        temp += "the skill level of the mage";
		}
		...
		if(spd->damage[i].effect) {
			...
			temp += EffectStr(spd->damage[i].effect);
                ...
                if(EffectDefs[spd->damage[i].effect].addhp > 0) {
                        temp += " This effect giving second life to ally soldiers.";
                }
                if(EffectDefs[spd->damage[i].effect].addattack > 0) {
                        temp += " This effect added ability second attack to ally soldiers.";
                }
	}
        if(spd->numrounds != 100) {
                temp += AString(" This spell casting ") + spd->numrounds +
                " rounds in a battle.";
        }
	...
	*temp += AString(illusion?"illusory ":"")+ ItemDefs[item].name + " [" +
	...
	if(Globals->ALLOW_WITHDRAW) {
		if((ItemDefs[item].type & IT_NORMAL && item != I_SILVER) ||
                        (ItemDefs[item].type & IT_MAN)) {
			*temp += AString(", costs ") + (ItemDefs[item].baseprice*3/2) +
				" silver to withdraw";
		}
	}
	*temp += ".";

       	if(ItemDefs[item].type & IT_ARTIFACT) *temp += " This is a artifact item.";
		...
		*temp += AString(" This monster attacks with a combat skill of ") +
		...
		if(MonDefs[mon].spoiltype != -1) {
                    *temp += " This monster might have ";
                        if(MonDefs[mon].spoiltype & IT_ARTIFACT) {
                                *temp += "artifact items and ";
		        } else if(MonDefs[mon].spoiltype & IT_MAGIC) {
		...
		if(MonDefs[mon].msup > 0) {
                    if (MonDefs[mon].numsup == 1) {
                       *temp += AString(" This monster demands ") + MonDefs[mon].msup +
                                                     " mana per month for maintnance.";
                    } else {
                       *temp += AString(" Maintnance of ") + MonDefs[mon].numsup +
                                                   " " + ItemDefs[item].names + " is " +
                                                   MonDefs[mon].msup + " mana per month.";
                    }
                }
	}
	...
	if(ItemDefs[item].flags & ItemType::MANPRODUCE) {
		...
		*temp += AString(" This FMI attacks with a combat skill of ") +
			MonDefs[mon].attackLevel + ".";
		for(int c = 0; c < NUM_ATTACK_TYPES; c++) {
			*temp += AString(" ") + FMIResist(c,MonDefs[mon].defense[c], full);
		}
		if(MonDefs[mon].special && MonDefs[mon].special != -1) {
			*temp += AString(" ") +
				"FMI can do " +
	...
	if(ItemDefs[item].type & IT_ARMOR) {
                if(ArmorDefs[ItemDefs[item].index].flags & ArmorType::ISCLOAK) {
        		*temp += " This is a type of cloak armor.";
                }
                else    *temp += " This is a type of armor.";
	...
	if(ItemDefs[item].type & IT_MOUNT) {
		*temp += " This is a mount.";
		...
		}/* else if(item == I_WHORSE) {
				*temp += AString(" This mount requires ") + SkillStrs(pM->skill) +
				" of at least level " + pM->minBonus + " to fly.";
				} */
			*temp += AString(" This mount requires ") + SkillStrs(pM->skill) +
	...
	if(ItemDefs[item].mSkill != -1 &&
		...
		*temp += AString(" Units with ") + SkillStrs(ItemDefs[item].mSkill) +
			" of at least level " + ItemDefs[item].mLevel;
                if((item >= I_CLOAKOFREFLECTION) && (item <= I_CLOAKOFSTABILITY)) {
                        switch(item) {
                                case I_CLOAKOFREFLECTION:
                                        *temp += AString(" and with Force Shield of level 3");
                                        break;
                                case I_CLOAKOFPROTECTION:
                                        *temp += AString(" and with Spirit Shield of level 3");
                                        break;
                                case I_CLOAKOFSHIELD:
                                        *temp += AString(" and with Energy Shield of level 3");
                                        break;
                                case I_CLOAKOFSTABILITY:
                                        *temp += AString(" and with Clear Skies of level 3");
                                        break;
                        }
                }
		*temp += AString(" may attempt to create this item via magic");
