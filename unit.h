...
class Unit : public AListElem
{
	public:
		...
		int CanStudy(int);
		int Study(int,int,bool p=0); /* Returns 1 if it succeeds */
		int Practise(int);
		void AdjustSkills();
		int AddCostForNumSkill(int); /* skill */        // VVR
		void CheckMergeMax();                           // VVR
		...
		int CanSwim();
		int CanReallySwim();
		int MoveType(ARegion *,ARegion *, int);
		int CalcMovePoints(ARegion *,ARegion *, int);
		...
		int num;
		int type;
		int npctype;                    // Beholder
		int alias;
		int gm_alias; /* used for gm manual creation of new units */
		int guard; /* Also, avoid- see enum above */
		int reveal;
		int flags;
		int manause;     // if ==1 then used MPOT  Default = 0. ==2 - use personal pool
		int taxing;
		int movepoints;
		int canattack;
		int nomove;
		int vsmax;      // Unique skill that up to max (5 OR 6) // VVR
		int vsdays;     // VVR
		SkillList skills;
		ItemList items;
		int combat;
		...
		Order *stealorders;
		Order *monthorders;
                Order *monordmagestudy;         // VVR for study a mage skills in moving
