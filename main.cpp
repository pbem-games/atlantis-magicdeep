...
void usage()
{
	...
	Awrite("");
	Awrite("atlantis modoceans");
	Awrite("atlantis modhills");
	Awrite("");
	Awrite("atlantis export objects");
}
...
int main(int argc, char *argv[])
{
	...
	Awrite(AString("Atlantis MagicDeep Engine Version: ") +
			ATL_VER_STRING(CURRENT_ATL_VER));
	Awrite(AString("Ruleset ") + Globals->RULESET_NAME + ", Version: " +
	...
	do {
		...
		} else if(AString( argv[1] ) == "statistic") {
		// dsnlab
			if( argc == 2 ) {
				if( !game.OpenGame() ) {
					Awrite( "Couldn't open the game file!" );
		...
		} else if(AString( argv[1] ) == "exportobj") {
		// VVR
			if( argc == 2 ) {
				if( !game.OpenGame() ) {
					Awrite( "Couldn't open the game file!" );
		...
		// Klaus
			if( argc == 2 ) {
				...
				game.GameFPStatistic();
			}
			else {
				usage();
				break;
			}
		} else if(AString(argv[1])== "modoceans") {
		// Klaus
			if( !game.OpenGame() ) {
				Awrite( "Couldn't open the game file!" );
				break;
		...
		} else if(AString(argv[1])== "modhills") {
		// Klaus
			if( !game.OpenGame() ) {
				Awrite( "Couldn't open the game file!" );
				break;
