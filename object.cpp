...
int Object::CantDestroy()
{
	return (ObjectDefs[type].flags & ObjectType::CANTDESTROY);
}
...
int Object::IsMRoad()
{
	if (type >= O_MROADN && type <= O_MROADS) return 1;
	return 0;
}
...
AString *ObjectDescription(int obj)
{
	...
	*temp += AString(o->name) + ": This is a ";
	if(o->capacity) {
		if (o->shipClass) *temp += AString("ship of class ") + o->shipClass + ".";
                else *temp += AString("ship of undefined class. Any effects based on ship's class are unappliable.");
	} else {
		*temp += "building.";
	}
	...
	if(o->protect) {
		*temp += AString(" This structure provides defense to the first ") +
			o->protect + " men inside it.";
               *temp += AString(" It provides a +") + o->defenceBonusArray[ATTACK_COMBAT]+
               " defence bonus versus melee attacks, +" + o->defenceBonusArray[ATTACK_ENERGY] +
               " defence bonus versus energy attacks, +" + o->defenceBonusArray[ATTACK_SPIRIT] +
               " defence bonus versus spirit attacks, +" + o->defenceBonusArray[ATTACK_WEATHER] +
               " defence bonus versus weather attacks, +" + o->defenceBonusArray[ATTACK_RIDING] +
               " defence bonus versus riding attacks and +" + o->defenceBonusArray[ATTACK_RANGED] +
               " defence bonus versus ranged attacks.";
	} else {
		*temp += AString(" This structure does not provide any defense.");
        }

	if(o->capacity) {
		*temp += AString(" This ship has ") + o->capacity + " weight capacity. ";
                *temp += AString("This ship's movement speed is ") +o->shipSpeed + ".";
                if (o->flags & ObjectType::SHIP_HEAVY) {
                        *temp += AString(" This is a heavy ship");
                        if (Globals->SHIP_CLASS_MODIFERS & GameDefs::SAIL_HEAVY_SLOWED_ON_SHALLOWS)
                                *temp += AString(", movement costs are doubled when sailing terrain types other than deeps");
                        *temp += AString(".");
                }
                if (o->flags & ObjectType::SHIP_LIGHT) {
                        *temp += AString(" This is a light ship");
                        if (Globals->SHIP_CLASS_MODIFERS & GameDefs::SAIL_LIGHT_SLOWED3_BAD_WEATHER)
                                *temp += AString(", movement costs are tripled in bad weather");
                        if (Globals->SHIP_CLASS_MODIFERS & GameDefs::SAIL_LIGHT_SLOWED4_BAD_WEATHER)
                                *temp += AString(", movement costs are quadrupled in bad weather");
                        if (Globals->SHIP_CLASS_MODIFERS & GameDefs::SAIL_LIGHT_STOPPED_BAD_WEATHER)
                                *temp += AString(", it cannot sail in bad weather");
                        *temp += AString(".");
                }
                if (o->flags & ObjectType::SHIP_AIRBORN) {
                        *temp += AString(" This is an aerial ship");
	...
	if(o->item == I_WOOD_OR_STONE &&
		...
		buildable = 0;
	if(!buildable) {
		*temp += " This structure cannot be build by players.";
	} else {
		*temp += AString(" This structure is builded using ") +
