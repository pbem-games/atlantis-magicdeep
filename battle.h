...
class Battle : public AListElem
{
	public:
		...
		void GetSpoils(AList * losers , ItemList * losses, int ass);

		void WriteSides(ARegion *,Unit *,Unit *,AList *,AList *,int,
				ARegionList *pRegs );

		void WriteArmies(Army *, Army *);

		// this includes looting, healing, ammunition loss etc for non-tie battles.
      void DoWonBattleResults(Army * winners, Army * losers,
		...
		void UpdateShields(Army *);
                void CastEffects(Army *, Army *);     // VVR 
		void DoSpecialAttack( int round, Soldier *a, Army *attackers,
				Army *def, int behind, int specfl=0);
                                        // specfl - if 1 then enable cast first&one-round-effect
