...
void Market::PostTurn(int population,int wages, int race, int townPop)
{
	...
	float ratio = ItemDefs[item].baseprice / (float)Globals->BASE_MAN_COST;
		...
		if (item == I_LEADERS)
			amount = population / 200;
		else
			if(item == race)
			{
				amount = (population - townPop)/10 + townPop/30;
			}
			else
			{
				ratio = (ItemDefs[item].baseprice + getrandom(ItemDefs[item].baseprice/2)) / (float)Globals->BASE_MAN_COST;
				price = (int)(wages * 4 * ratio);
				if(townPop >= 2000)
				{
						amount = townPop/(80 + getrandom(40));
				}
				else
				{
					if(townPop >= 1000)
					{
						amount = townPop/(320 + getrandom(160));
					}
				}
				if(townPop == 5000)
				{
...
void MarketList::PostTurn(int population,int wages,int race,int town) {
  forlist(this) {
	((Market *) elem)->PostTurn(population,wages,race,town);
  }
}
